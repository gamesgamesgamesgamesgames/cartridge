...
import { ChevronLeft, ChevronRight, Heart, Info, Pause, Play } from 'lucide-react'
...
const PAN_DIRECTIONS = [
	{ from: 'scale(1.15) translate(-3%, -3%)', to: 'scale(1.15) translate(3%, 3%)' },
	{ from: 'scale(1.15) translate(3%, -3%)', to: 'scale(1.15) translate(-3%, 3%)' },
	{ from: 'scale(1.15) translate(-3%, 3%)', to: 'scale(1.15) translate(3%, -3%)' },
	{ from: 'scale(1.15) translate(3%, 3%)', to: 'scale(1.15) translate(-3%, -3%)' },
] as const
...
function SlideImage({ uri, mediaItem, isVisible, pan, reducedMotion }: {
	uri: string
...
function SlideBackgrounds({ game, isSlideActive, reducedMotion }: {
	game: GameFeedGame
...
function SlideContent({ slide, isActive }: {
	...
	return (
		...
		>
			<span className={'block text-xs font-semibold uppercase tracking-wider text-primary md:text-sm'}>
				{category}
			</span>
			...
			{href ? (
				<Link
					...
					className={'mt-1 inline-block focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary'}>
					<span className={'block text-3xl font-black text-white [text-wrap:balance] md:text-5xl lg:text-6xl'}>
						{game.name}
					</span>
				</Link>
			) : (
				<span className={'mt-1 block text-3xl font-black text-white [text-wrap:balance] md:text-5xl lg:text-6xl'}>
					{game.name}
				</span>
			)}
			...
			{(Boolean(game.genres?.length) || likeCount > 0) && (
				<div className={'mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5'}>
					{game.genres?.slice(0, 4).map((genre) => (
						<span key={genre} className={'rounded-full bg-white/15 px-2.5 py-0.5 text-xs font-medium text-white/90'}>
			...
			{description && (
				<p className={'mt-4 hidden line-clamp-3 max-w-xl text-sm text-white/90 [text-wrap:pretty] md:block md:text-base'}>
					{description}
				</p>
			)}
			...
			{href && (
				<div className={'mt-5'}>
					<Button asChild size={'lg'} tabIndex={isActive ? undefined : -1}>
						<Link href={href}>
...
function DotIndicator({ isActive, duration, reducedMotion }: {
	isActive: boolean
	duration: number
	reducedMotion: boolean
}) {
	...
	return (
		<span className={'relative block h-2 w-full overflow-hidden rounded-full'}>
			<span className={`absolute inset-0 rounded-full transition-colors duration-300 ${
				isActive ? (reducedMotion ? 'bg-primary' : 'bg-white/20') : 'bg-white/15 group-hover:bg-white/30'
			}`} />
...
function HeroCarousel({ slides }: {
	...
	return (
		...
		>
			<div className={'relative aspect-[16/9] max-h-dvh w-full md:aspect-[21/9]'}>
				{/* Backgrounds — stacked via grid, image cycling + Ken Burns per slide */}
				...
				{/* Content */}
				<div className={'absolute inset-x-0 bottom-0 px-4 md:px-10 lg:px-16'}>
								...
								<div className={'mt-2 flex items-center gap-1.5 md:mt-4'}>
									{slides.map((_, i) => (
										<button
											...
											className={'group flex min-h-11 w-8 items-center justify-center'}
											onClick={() => goTo(i)}>
											<DotIndicator
												isActive={i === activeIndex}
									...
									{!reducedMotion && (
										<button
											...
											className={'ml-1 flex min-h-11 min-w-11 items-center justify-center rounded-full text-white/50 transition-colors hover:text-white'}
											onClick={togglePause}>
											{paused
				...
				{count > 1 && (
					<>
						<button
							...
							className={'absolute left-3 top-1/2 z-10 -translate-y-1/2 rounded-full bg-black/30 p-2.5 text-white/60 transition-colors hover:bg-black/50 hover:text-white md:left-4 md:p-3'}
							onClick={goPrev}>
						...
						<button
							...
							className={'absolute right-3 top-1/2 z-10 -translate-y-1/2 rounded-full bg-black/30 p-2.5 text-white/60 transition-colors hover:bg-black/50 hover:text-white md:right-4 md:p-3'}
							onClick={goNext}>
