...
import { type GameSummaryView, type ProfileSummaryView } from '@/helpers/lexicons/games/gamesgamesgamesgames/defs.defs'
...
export function ClaimPage() {
	...
	const [searchResults, setSearchResults] = useState<Array<GameSummaryView | ProfileSummaryView>>([])
	...
	const runSearch = useCallback(
		async (q: string, currentMode: ClaimMode) => {
			if (!q.trim()) {
				setSearchResults([])
				return
			}
			...
			try {
				...
				const result = await API.search(q, { types, limit: 10 })
				setSearchResults(
	...
	const handleGameToggle = useCallback(
		(game: GameSummaryView) => {
			setSelectedGames((prev) => {
				...
				return [...prev, { uri: game.uri, name: game.name }]
			})
		},
		[],
	)
	...
	const handleJustificationChange = useCallback<ChangeEventHandler<HTMLTextAreaElement>>(
		...
		return (
			<Container>
				<div className={'flex flex-col gap-4 max-w-lg'}>
					<h1 className={'text-2xl font-bold'}>{'Claim Games'}</h1>
					...
					<div className={'rounded-lg border border-border bg-card p-6 flex flex-col gap-4'}>
						...
						<div className={'flex gap-3 flex-wrap'}>
							<Button
								asChild
								variant={'default'}>
								<Link href={'/claim/list'}>{'View My Claims'}</Link>
							</Button>
	...
	const hasSelection = mode === 'game' ? selectedGames.length > 0 : selectedOrg !== null
			...
			<div className={'flex flex-col gap-8 max-w-lg'}>
				...
				<div className={'flex flex-col gap-3'}>
					...
					<ToggleGroup
						...
						<ToggleGroupItem value={'game'}>{'Claim Games'}</ToggleGroupItem>
						<ToggleGroupItem value={'org'}>{'Claim Organization'}</ToggleGroupItem>
					</ToggleGroup>
				</div>
				...
				<div className={'flex flex-col gap-3'}>
					<Label htmlFor={'search'}>
						{mode === 'game' ? 'Search for Games' : 'Search for Organization'}
					</Label>
						...
						<div className={'rounded-md border border-border divide-y divide-border overflow-hidden'}>
							{mode === 'game' &&
								(searchResults as GameSummaryView[]).map((game) => {
									const isSelected = selectedGames.some((g) => g.uri === game.uri)
									return (
										<button
											key={game.uri}
											type={'button'}
				...
				{mode === 'game' && selectedGames.length > 0 && (
					<div className={'flex flex-col gap-3'}>
